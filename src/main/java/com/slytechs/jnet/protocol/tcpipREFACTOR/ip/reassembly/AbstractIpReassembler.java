...
public abstract class AbstractIpReassembler implements StatefulIpf {
	...
	public void handleIpf(
			...
			int caplen,
			int wirelen,
			IpfFragmentDescriptor ipfDescriptor) {
		...
		if (!validateFragment(ipfDescriptor)) {
			...
			notifyEvent(IpfEventType.ERROR_INVALID_FRAGMENT, ipfDescriptor);
			return;
		}
		...
		if (context == null) {
			stats.onTableFull();
			notifyEvent(IpfEventType.ERROR_TABLE_FULL, ipfDescriptor);
			return;
		}
	 ...
	 */
	protected abstract boolean validateFragment(IpfFragmentDescriptor descriptor);

	/**
	 * Gets or creates a reassembly context for the fragment.
	 */
	protected IpReassemblyContext getReassemblyContext(IpfFragmentDescriptor descriptor, long timestamp) {
		...
		if (context == null) {
			...
			notifyEvent(IpfEventType.REASSEMBLY_STARTED, descriptor);
		}
	 ...
	 */
	protected abstract FragmentKey createFragmentKey(IpfFragmentDescriptor descriptor);
	...
	protected void processFragment(
			...
			MemorySegment packetSegment,
			ByteBuffer packetBuffer,
			IpfFragmentDescriptor descriptor,
			long timestamp) {

		// Check for duplicate fragment
		if (context.hasFragment(descriptor)) {
			stats.onDuplicateFragment();
			notifyEvent(IpfEventType.DUPLICATE_FRAGMENT, descriptor);
			return;
		}
		...
		notifyEvent(IpfEventType.FRAGMENT_ACCEPTED, descriptor);
	}
	...
	protected void completeReassembly(IpReassemblyContext context, long timestamp) {
		...
		notifyEvent(IpfEventType.REASSEMBLY_COMPLETE, context.getDescriptor());
	}
	...
	protected void cleanupExpiredEntries(long currentTime) {
		...
		reassemblyTable.forEach((key, context, hash, index) -> {
			if (currentTime - context.getLastUpdateTime() > timeout) {
				...
				notifyEvent(IpfEventType.REASSEMBLY_TIMEOUT, context.getDescriptor());
	 ...
	 */
	protected void notifyEvent(IpfEventType type, IpfFragmentDescriptor descriptor) {
